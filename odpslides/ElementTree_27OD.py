...
# 2015-09-22 ct   added OrderedDict and removed sorted attrib
#
...
class Element(object):
    ...
    def __init__(self, tag, attrib=OrderedDict(), **extra):
        attrib = attrib.copy()
        attrib.update(extra)
        self.tag = tag
        self.attrib = attrib
        ...
        if tag is None:
            ...
            if items or namespaces:
                ...
                for k, v in items:  # lexical order
                    ...
                    # cet Added logic to catch unicode badness
                    try:
                        write(" %s=\"%s\"" % (qnames[k], v))
                    except UnicodeDecodeError:
                        #print( 'type(v)=',type(v), b'%s'%v )
                        #print( 'type(k)=',type(k), b'%s'%k )
                        v = v.decode('utf-8')
                        #write(" %s=\"%s\"" % (qnames[k], v.encode('utf-8')))
                        write(" %s=" % (qnames[k], ))
                        write(b"\"%s\"" % (v.encode('utf-8'),))
...
class XMLParser(object):
    ...
    def __init__(self, html=0, target=None, encoding=None):
        ...
        self._names = OrderedDict() # name memo cache
        # callbacks
