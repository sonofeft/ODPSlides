...
if __name__ == "__main__":
    ...
    #for suffix in ['plain']:
        ...
        styles_style_name_lookupD.clear()
    
        fname = r'D:\py_proj_2015\ODPSlides\odpslides\templates\ppt_all_layouts_%s.odp'%suffix
        odp = ODPFile( fname )
        
        py_file = os.path.join(here, suffix, 'styles_auto_styles.py')
...
''' )
        ...
        fOut.write('\n\n# Set values in styles_style_name_lookupD\n')
        def int_part( s ):
            try:
                val = int(s[1:])
            except:
                val = 0
            return val
        keyL = sorted( styles_style_name_lookupD.keys(), key=int_part )
        for key in keyL:
            fOut.write( 'styles_style_name_lookupD["%s"] = %s\n'%(key, styles_style_name_lookupD[key]) )
        
        
        fOut.write('''
if __name__ == "__main__":
    print( styles_style_name_lookupD["a100"]() )
''')
